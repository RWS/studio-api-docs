...
```
Next apply the TranslationMemories property to the selected template and loop through all TMs associated with the template:
```
...
```
Last, save the template, thereby actually creating it on the server. Optionally, you may use the `IsDirty` boolean property to verify whether there are any unsaved changes in your template definition, which should not be the case:
...
Fields templates can be deleted by applying the Delete method:
