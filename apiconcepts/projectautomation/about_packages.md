...
When the task assignee receives the project package, it can be opened in Trados Studio 2017 in order to perform the manual task. When opening a project package in Trados Studio 2017 , the contents of the package are extracted, and a Trados Studio 2017 project is created based on the system of the assignee. Except for the fact that this project was created by opening a package rather than through the **Create Project Wizard**, it is standard Trados Studio 2017 project, i.e. the same functionality is available as for a 'normal' project, which was created in the **New Project** wizard.

After performing the manual task, for instance by translating the project files in the Trados Studio 2017 translation editor, the user selects the translated files and creates a **return package** in order to send the translated or edited files back. Like a project package, a return package is a ZIP file containing an XML manifest file and the translated project files. For more information on the contents of return packages, see Return Packages.
...
**Centralized Scenario** For example:
...
**Collaborative Scenario** For example:
...
1. **Working with packages within the context of a Project API-based project**: The Project Automation API provides a high-level API for creating and opening packages within the context of a project. See High-level API.
...
The creation of project packages in SDL Trados Studio 2017 is fully wizard-based. Users define the assignee, the due date, the type of manual task to perform, etc.
