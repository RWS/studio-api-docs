...
This chapter explains how to implement a class for controlling the plug-in user interface. This class acts as an intermediary between the plug-in UI and the class for setting and retrieving settings values (see [Retrieve the Settings Values](retrieve_the_settings_values.md)).

### Add the Class for Controlling the User Interface
Add a new class to your project and name it, for example, `IdenticalVerifierUIPage.cs`. This class contains several overriding methods required for controlling the user interface you created previously (see [Implement the User Interface](implement_the_user_interface.md)). This class needs to reference the namespaces `Sdl.Core.Settings` and `Sdl.Verification.Api`. Moreover, the new class needs to be derived from [AbstractSettingsPage](../../api/core/Sdl.Core.Settings.AbstractSettingsPage.yml).
...
Note that there are various strings to provide in the annotation, such as the plug-in page name and description. The plug-in page name corresponds to the link clicked in the user interface of Var:ProductName to display the corresponding page.
...
Next, add the following member, which is used to get a handle on the plug-in user interface (i.e. the control).
...
* load settings configured in a previous session
* determine what happens when the user clicks the **OK** button, thereby saving the configured settings
* define what happens when the user clicks **Reset to Defaults**, thereby restoring the settings to their default values
* move away from the settings page, thereby cancelling the settings page operation
...
The complete class for controlling the settings page should now be as shown below:
