...
This chapter explains how to implement the extension class that contains the actual verification functionality of your plug-in.
...
This class must be preceded by the following declaration, which makes it an extension class referenced in the plug-in manifest (see also [Create a New Project](create_a_new_project.md)).
...
This line is what makes the plug-in appear under **Verification** in the **Options** dialog box or in the **Project (Template) Settings** dialog box of Var:ProductName.
This class must implement the interfaces listed below:
...
Moreover, add a private member to the class, for example `_verificationSettings`. This object is derived from the `IdenticalVerifierSettings` class (see [Retrieve the Settings Values](retrieve_the_settings_values.md)) and is used to access the plug-in settings that influence how the actual verification is executed:
